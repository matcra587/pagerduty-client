...
func TestConfigToMap_FullyPopulated(t *testing.T) {
	...
	assert.Equal(t, "dark", m["tui.theme"])
	assert.Equal(t, "true", m["tui.show_resolved"])
	assert.Equal(t, "50", m["tui.page_size"])
	assert.Equal(t, "incidents,services", m["tui.tabs"])
}
...
func TestParseConfigValue(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "page_size valid", key: "tui.page_size", val: "25", want: 25},
		{name: "page_size zero", key: "tui.page_size", val: "0", wantErr: true},
		...
		{name: "show_resolved true", key: "tui.show_resolved", val: "true", want: true},
		{name: "tabs single", key: "tui.tabs", val: "incidents", want: []string{"incidents"}},
		{name: "tabs multiple", key: "tui.tabs", val: "incidents,services,teams", want: []string{"incidents", "services", "teams"}},
		{name: "tabs with spaces", key: "tui.tabs", val: "incidents, services", want: []string{"incidents", "services"}},
		{name: "tabs empty", key: "tui.tabs", val: "", wantErr: true},
	}
...
func TestModifyConfigFile_TUISection(t *testing.T) {
	t.Parallel()

	dir := t.TempDir()
	path := filepath.Join(dir, "config.toml")

	require.NoError(t, modifyConfigFile(path, "tui.theme", "light"))
	require.NoError(t, modifyConfigFile(path, "tui.show_resolved", true))
	require.NoError(t, modifyConfigFile(path, "tui.page_size", 50))
	require.NoError(t, modifyConfigFile(path, "tui.tabs", []string{"incidents", "services"}))

	cfg, err := config.Load(config.WithPath(path))
	require.NoError(t, err)
	assert.Equal(t, "light", cfg.TUI.Theme)
...
func TestRemoveConfigKey_NestedCleansSection(t *testing.T) {
	...
	require.NoError(t, modifyConfigFile(path, "tui.theme", "dark"))

	require.NoError(t, removeConfigKey(path, "tui.theme"))
...
`
	...
	assert.Contains(t, sources, "tui.theme")
...
func TestSplitConfigKey(t *testing.T) {
	...
	tests := []struct {
		...
		{key: "base_url", section: "", leaf: "base_url"},
		{key: "defaults.format", section: "defaults", leaf: "format"},
		{key: "tui.theme", section: "tui", leaf: "theme"},
	}

	for _, tt := range tests {
		t.Run(tt.key, func(t *testing.T) {
			t.Parallel()
			section, leaf := splitConfigKey(tt.key)
			assert.Equal(t, tt.section, section)
			assert.Equal(t, tt.leaf, leaf)
		})
	}
}
...
func TestRoundTrip_SetThenLoad(t *testing.T) {
	...
	require.NoError(t, modifyConfigFile(path, "tui.tabs", []string{"incidents", "teams"}))
	...
	assert.Equal(t, []string{"incidents", "teams"}, cfg.TUI.Tabs)
}
